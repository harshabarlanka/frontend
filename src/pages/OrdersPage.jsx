      ...
      <div className="flex flex-wrap gap-4 justify-between items-start mb-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <p className="font-body font-bold text-earth-900 text-sm">{order.orderNumber}</p>
            <span className={`badge ${statusInfo.color}`}>{statusInfo.label}</span>
            {order.paymentMethod === 'cod' && (
              <span className="badge bg-amber-100 text-amber-800">COD</span>
            )}
          </div>
          <p className="font-body text-xs text-earth-400">Placed on {formatDate(order.createdAt)}</p>
        </div>
        <div className="text-right">
          <p className="font-display font-bold text-earth-900 text-lg">{formatPrice(order.total)}</p>
          <p className="font-body text-xs text-earth-400">{order.items?.length} item(s)</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-4 overflow-hidden">
        ...
        {order.items?.length > 4 && (
          <div className="w-12 h-12 rounded-lg bg-earth-100 flex-shrink-0 flex items-center justify-center">
            <span className="font-body text-xs text-earth-500 font-bold">+{order.items.length - 4}</span>
          </div>
        )}
      </div>
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step} className="flex items-center flex-1 last:flex-none">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold font-body flex-shrink-0 transition-all ${
              done ? 'bg-leaf-500 text-white' : active ? 'bg-brand-500 text-white ring-4 ring-brand-100' : 'bg-earth-200 text-earth-400'
            }`}>
              {done ? '✓' : stepNum}
            </div>
