...
export const CartProvider = ({ children }) => {
  ...
  const addComboToCart = useCallback(async (comboId, quantity = 1) => {
    const { data } = await addComboToCartAPI({ comboId, quantity });
    setCart(data.data.cart);
  ...
  return (
    <CartContext.Provider
      value={{
        ...
        fetchCart, addToCart, addComboToCart, updateQuantity, removeItem, clearCart,
      }}
    >
      {children}
    </CartContext.Provider>
